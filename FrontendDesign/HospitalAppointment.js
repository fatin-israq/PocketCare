...
// API Service Layer (Replace with actual API calls)
const appointmentService = {
  fetchAppointments: () => Promise.resolve({ data: [] }), // Replace with API
  fetchDoctors: () => Promise.resolve({ data: [] }), // Replace with API
  createAppointment: (data) => Promise.resolve({ data }), // Replace with API
  updateAppointment: (id, data) => Promise.resolve({ data }), // Replace with API
  deleteAppointment: (id) => Promise.resolve({ success: true }), // Replace with API
};

// Configurations (Can be moved to separate file)
const CONFIG = {
  departments: [
...
const HospitalAppointments = () => {
  ...
  const filteredAppointments = state.appointments.filter(apt => {
    ...
    const matchesDate = !state.filters.date || 
      apt.date === moment(state.filters.date).format('YYYY-MM-DD');
    
    return matchesSearch && matchesStatus && matchesDoctor && 
           matchesDepartment && matchesDate;
  });
  ...
  const calendarEvents = state.appointments.map(apt => ({
    id: apt.id,
  ...
  return (
    <div className="max-w-7xl mx-auto p-4">
        ...
        <div className="flex flex-wrap gap-4 items-center">
          ...
          <button
            ...
            onClick={() => setState(prev => ({
              ...prev,
              viewMode: prev.viewMode === 'list' ? 'calendar' : 'list'
            }))}
          >
      ...
      {state.viewMode === 'calendar' ? (
        <div className="bg-white p-4 rounded-lg shadow mb-6 h-[600px]">
          <Calendar
            localizer={localizer}
            ...
            style={{ height: '100%' }}
            onSelectEvent={(event) => {
              const apt = state.appointments.find(a => a.id === event.id);
              setState(prev => ({ ...prev, formData: apt, showForm: true }));
            }}
          />
        </div>
      ) : (
